...
RestClient::response RestClient::post(const std::string& url,
									  ...
									  bool bUseIEProxy/* = true*/)
{
#ifdef _DEBUG
	std::string sAcc = "postData:" + data + "\r\n";
	OutputDebugStringA(sAcc.c_str());
#endif
...
#ifdef _DEBUG
	  OutputDebugStringA("Failed to query\r\n");
#endif
...
std::string RestClient::getIEProxy()
{
	...
	if(!WinHttpGetIEProxyConfigForCurrentUser(&MyProxyConfig))
	{
		...
		std::cout << "WinHttpGetIEProxyConfigForCurrentUser failed with the following error number: " << Err << std::endl;
		switch (Err)
		{
		case ERROR_FILE_NOT_FOUND:
			std::cout << "The error is ERROR_FILE_NOT_FOUND" << std::endl;
			break;

		case ERROR_WINHTTP_INTERNAL_ERROR:
			std::cout << "ERROR_WINHTTP_INTERNAL_ERROR" << std::endl;
			break;
		...
		default:
			std::cout << "Look up error in header file." << std::endl;
		}//end switch
	...
	else
	{
		...
		std::cout << "Auto Detect is: " << MyProxyConfig.fAutoDetect << std::endl;

		if(NULL != MyProxyConfig.lpszAutoConfigUrl)
		{
			std::cout << "AutoConfigURL is: " << MyProxyConfig.lpszAutoConfigUrl << std::endl;
			std::string sProxy(ws2s(std::wstring(MyProxyConfig.lpszAutoConfigUrl)));
		...
		{
			std::cout << "AutoConfigURL is: " << MyProxyConfig.lpszProxy << std::endl;
			std::string sProxy(ws2s(std::wstring(MyProxyConfig.lpszProxy)));
		...
		{
			std::cout << "AutoConfigURL is: " << MyProxyConfig.lpszProxyBypass << std::endl;
			std::string sProxy(ws2s(std::wstring(MyProxyConfig.lpszProxyBypass)));
...
std::string RestClient::ws2s( const std::wstring& ws )
{
...
std::wstring RestClient::s2ws( const std::string& s )
{
	setlocale(LC_ALL, "chs"); 
