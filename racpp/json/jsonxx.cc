...
std::map<std::string, Value*>& Object::kv_map(){
	return value_map_;
}
...
bool Array::import(int index, const Array &other)
{
	int iSize = size();
	...
	if (this != &other) {
		// default
		container::const_iterator
			beginIt = other.values_.begin(),
			endIt = other.values_.end();

		container::const_iterator
			it = other.values_.begin(),
			end = other.values_.end();

		container values_tmp;
		for (/**/ ; it != end; ++it) {
			values_tmp.push_back( new Value(**it) );
		}

		values_.insert(values_.begin()+index, values_tmp.begin(), values_tmp.end());
		return true;

	} else {
		// recursion is supported here
		return import( index, Array(*this) );
	}
}

bool Array::import(int index, const Value &value)
{
	int iSize = size();
	if (index < 0 || index > iSize)
	{
	...
	values_.insert(values_.begin()+index, new Value(value));
	return true;
}
